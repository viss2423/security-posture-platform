...
- Images are pinned to immutable digest placeholders (`@sha256:...`); replace each placeholder digest with the signed release digest for your registry.
- CI guardrail: `python services/api/scripts/check_k8s_image_pinning.py --require-real-digests` enforces non-placeholder digests.
