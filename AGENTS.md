...
This project is indexed by GitNexus as **security-posture-platform** (6739 symbols, 15843 relationships, 300 execution flows, last refreshed 2026-07-13). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- **MUST treat an unresolved impact/context result as a stale index, not as low risk.** If `impact()` or `context()` returns "not found," `UNKNOWN`, or a non-`"exact"` `epistemic` value for a symbol you know exists in the repo (e.g. you just read it, or another agent just wrote it), do not proceed on that result. Run `node .gitnexus/run.cjs analyze` first, then retry the query. This has caused a real incident: an agent got `UNKNOWN` on several symbols after a batch of uncommitted edits, treated it as equivalent to a clean LOW-risk result, and proceeded — the index was simply stale (last analyzed before those edits). A blocking git hook to auto-refresh on every commit was tried once and reverted (`6e6d1c3`, hung on Windows) — so this is a manual check, not automatic; don't assume freshness.
