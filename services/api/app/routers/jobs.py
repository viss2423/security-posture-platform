"""Job runner: scan_jobs table, list/get/retry, logs. Phase B.3."""
...
def get_job(
    ...
    row = db.execute(
        text("SELECT job_id, job_type, target_asset_id, requested_by, status, created_at, started_at, finished_at, error, log_output, retry_count FROM scan_jobs WHERE job_id = :id"),
        {"id": job_id},
    ).mappings().first()
    if not row:
...
def retry_job(
  ...
  _user: str = Depends(require_role(["admin", "analyst"])),
    ...
    row = db.execute(text("SELECT job_id, status FROM scan_jobs WHERE job_id = :id"), {"id": job_id}).mappings().first()
    if not row:
        raise HTTPException(status_code=404, detail="Job not found")
