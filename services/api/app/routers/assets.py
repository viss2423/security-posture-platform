...
def _redact_asset(row: dict, role: str) -> dict:
    out = dict(row)
    if role == "viewer":
        for key in SENSITIVE_ASSET_FIELDS_FOR_VIEWER:
            if key == "metadata":
                out[key] = {}
            else:
                out[key] = None
...
def get_asset(
    ...
    return _redact_asset(dict(row), role=role)


@router.get("/by-key/{asset_key}")
def get_asset_by_key(
    asset_key: str,
    db: Session = Depends(get_db),
    _user: str = Depends(require_auth),
    role: str = Depends(get_current_role),
...
def update_asset_by_key(
    ...
    """
    exists = (
        db.execute(text("SELECT asset_id FROM assets WHERE asset_key=:k"), {"k": asset_key})
        .mappings()
        .first()
    )
    if not exists:
