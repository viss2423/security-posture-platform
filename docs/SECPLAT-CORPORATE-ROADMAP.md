...
- A Phase 5 baseline manifest set now exists in `infra/k8s/`:
  - Deployments: `secplat-api`, `secplat-deriver`, `secplat-worker-web`, `secplat-notifier`, `secplat-correlator`
  - `HorizontalPodAutoscaler` for `secplat-worker-web`
  - CronJobs for ingestion + report snapshots
  - NetworkPolicies and pod/container security context hardening
